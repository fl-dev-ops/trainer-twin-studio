...
Matches the OpenAI-compatible speech streaming protocol used by TrainerTwin:
    POST /api/tts/speech (or /v1/audio/speech)
    ...
    async def _run(self, output: tts.AudioEmitter) -> None:
        ...
        try:
            async with vox.session().post(
                ...
                output.flush()
                logger.info(
                    "[TTS:voxcpm2] complete request_id=%s elapsed_ms=%.1f audio_bytes=%d",
                    request_id,
            ...
            raise APIConnectionError(
                f"VoxCPM2 TTS connection failed: {error}",
...
def build_voxcpm2_tts(*, voice: str = "") -> VoxCPM2TTS:
    ...
    resolved_voice = (
        voice
        or os.getenv("AGENT_VOICE_ID_OVERRIDE")
