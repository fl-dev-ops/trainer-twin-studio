...
```json
{
  ...
  "practiceUrl": "https://your-org.trainertwin.com/s/AbC123",
  "created": true,
  "emailSent": true
}
```

`practiceUrl` is the learner's direct practice link — send it through your own channel instead of (or alongside) the TrainerTwin email.
...
A changed assignment sends a notification to the target learner and returns a new `practiceUrl` (the old link stops working). A no-op update returns `changed: false`, the existing `practiceUrl`, and sends no email.
...
Returns the learner, scenario, current `status` (`pending`, `used`, `expired`, or `cancelled`), and `practiceUrl`.
