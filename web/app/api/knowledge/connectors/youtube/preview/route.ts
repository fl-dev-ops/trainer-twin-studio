...
import { previewYouTubeImport } from "@/lib/youtube-ingestion";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[API:connectors:youtube:preview] failed:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to preview YouTube video" },
      { status: 500 },
    );
  }
}
