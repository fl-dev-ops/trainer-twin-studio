...
export const PromptInputActionAddAttachments = ({
  ...
  const handleSelect = useCallback(
    (e: Parameters<NonNullable<PromptInputActionAddAttachmentsProps["onSelect"]>>[0]) => {
      e.preventDefault();
...
export const PromptInputActionAddScreenshot = ({
  ...
  const handleSelect = useCallback(
    async (event: Parameters<NonNullable<PromptInputActionAddScreenshotProps["onSelect"]>>[0]) => {
      onSelect?.(event);
      ...
      try {
        ...
        const convertedFiles: FileUIPart[] = await Promise.all(
          files.map(async (file) => {
            const item = { ...file };
            delete (item as Partial<typeof item>).id;
