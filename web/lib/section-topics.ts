...
import { createTopicResolver } from "./topic-normalization";
...
export function applySectionTopics(
  ...
  for (const sectionId of failed) bySection.get(sectionId)?.clear();
  return document.chunks.map((chunk) => ({
    ...chunk,
    topics: [...new Set(chunk.sectionIds.flatMap((id) => [...(bySection.get(id) ?? [])]))],
  }));
}
