...
export async function finishYouTubeOAuth(
  ...
  const state = await db.youTubeOAuthState.findFirst({
    where: { id: stateId, orgId, userId },
  ...
  if (consumed.count !== 1) {
    throw new Error("YouTube authorization already used");
  }

  if (params.has("error")) {
    return { kbId: state.kb.id, kbSlug: state.kb.slug, status: "cancelled" };
  }
  ...
  for (const channel of channels) {
    const where = { orgId_channelId: { orgId, channelId: channel.id } };

    const existing = await db.youTubeConnection.upsert({
      where,
      update: {},
      create: {
        id: randomUUID(),
    ...
    const saved = await db.youTubeConnection.updateMany({
      where: { id, orgId, status: { not: "disconnecting" } },
      data: credentials,
    });
  ...
  return { kbId: state.kb.id, kbSlug: state.kb.slug, status: "connected" };
}
