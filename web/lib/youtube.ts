...
export const youtubeImportSchema = z
  .object({
    url: z
      ...
      .max(2048)
      .refine((val) => Boolean(parseYouTubeVideoId(val)), {
...
export const youtubeRefreshDocumentSchema = z
  .object({
...
export const youtubeDisconnectSchema = z
  .object({ connectionId: z.string().min(1).max(128) })
  .strict();
...
export const youtubeStateSchema = z.object({
  ...
  jobs: z.array(
    z.object({
      ...
      segmentsTotal: z.number().int().nonnegative(),
      segmentsSucceeded: z.number().int().nonnegative(),
      segmentsFailed: z.number().int().nonnegative(),
    }),
  ),
});
