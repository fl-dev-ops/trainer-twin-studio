import { AdminClient, AdminCloudClient, ChromaClient, CloudClient, type Collection, type EmbeddingFunction } from "chromadb";
...
function getAdminClient(config: PipelineConfig): AdminClient | AdminCloudClient {
  if (config.chromaCloud) {
    return new AdminCloudClient({ apiKey: config.chromaCloud.apiKey });
  }
  return new AdminClient(chromaConnection(config.chromaUrl));
}

export async function ensureOrgDatabase(
  config: PipelineConfig,
  tenant: string,
  database: string,
): Promise<void> {
  if (database === "default_database" || database === "default" || (config.chromaCloud && database === config.chromaCloud.database)) return;
  try {
    ...
    console.warn(`[Chroma] ensureOrgDatabase warning for tenant=${tenant} database=${database}:`, error);
  }
}
...
function createChromaClient(config: PipelineConfig, tenant = "default_tenant", database = "default_database"): ChromaClient {
  if (config.chromaCloud) {
    return new CloudClient({
      apiKey: config.chromaCloud.apiKey,
      tenant,
      database,
    });
  }
...
export async function getMainCollection(
  ...
  const assignedTenant = tenantId || config.chromaCloud?.tenant || "default_tenant";
  const defaultCloudDb = config.chromaCloud?.database;
  const assignedDatabase = database && database !== "default" && database !== "default_database"
    ? database
    : (defaultCloudDb || orgDatabaseName(orgId));
