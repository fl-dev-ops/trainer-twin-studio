...
import { orgDatabaseName } from "./knowledge";
...
async function loadContext(context: Pool, message: IngestionMessage): Promise<{ job: JobContext; workItem: WorkItemContext } | null> {
  const result = await context.query<JobContext & WorkItemContext & { orgExists: boolean }>(`
    ...
    WHERE job.id = $1 AND item.id = $2 AND source.connector IN ('upload', 'notion', 'notion_public', 'youtube')
  ...
  const chromaDatabase = row.chromaDatabase && row.chromaDatabase !== "default" && row.chromaDatabase !== "default_database"
    ? row.chromaDatabase
    : orgDatabaseName(row.orgId);
  const chromaTenantId = row.chromaTenantId || (process.env.CHROMA_TENANT?.trim() || null);

  if (row.orgExists && (!row.chromaDatabase || row.chromaDatabase === "default" || !row.chromaTenantId)) {
    context.query(
      `UPDATE "organization" SET "chromaTenantId" = COALESCE("chromaTenantId", $1), "chromaDatabase" = $2 WHERE id = $3`,
      [chromaTenantId, chromaDatabase, row.orgId],
    ).catch(() => {});
  }
